...
const Hero = () => {
  ...
  return ( 
    ...
    className='h-screen flex flex-col items-center justify-content-center gap-14 bg-light text-center'>
      
     <motion.h1 initial={{ y:50, opacity:0}}
       animate={{ y:0, opacity:1}} 
       transition={{duration:0.8, delay:0.2}} 
       
       className="pt-10 text-4xl md-text-5xl font-semibold ">Luxury Cars on Rent
      </motion.h1>
       <motion.form
       ...
       onSubmit={handleSearch} className='flex flex-col md:flex-row items:start md:items-center justify-between p-6 rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white shadow-[0px_8px_20px_rgba(0,0,0,0.1)]'>
          <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
            ...
            <div className='flex flex-col items-start gap-2'>
              ...
              <input  min={pickupDate} disabled={!pickupDate} value={returnDate} onChange={e => setReturnDate(e.target.value)} type="date" id="return-date" className='text-sm text-gray-500 required'/>
            </div>
          </div>
              <motion.button  
              ...
              <img src={assets.search_icon} alt="search" className='brightness-300'></img>
              Search
            </motion.button>
       </motion.form>
